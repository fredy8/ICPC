...
mint main() {
	...
	vi p[2];
	int turn = 0;
	deque<pair<ii, int> > q, q2;
	q.push_front(mp(ii(0, N-1), 0));
	while(!q.empty() || !q2.empty()) {
		...
		if(range.second == range.first) {
			...
			FOR(i, range.first, range.second+1) {
				int start = i;
				while(i < range.second+1 && arr[i][col] == arr[start][col])i++;
				i--;
				q2.push_front(mp(ii(start, i), col+1));
			}
		}
